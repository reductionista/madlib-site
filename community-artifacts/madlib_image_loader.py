...
def _call_disk_worker(label):
    # TODO: -  call _load_from_disk()
    #        - split data into blocks of NUM_ROWS_PER_FILE
    #          rows each
    #        - call _call_np_worker on each block
    pass

def _call_np_worker(data):
    try:
...
def init_worker(mother_pid, table_name, append, no_temp_files,
                db_creds, root_dir=None):
    pr = current_process()
...
def main():
    ...
    parser.add_argument('-n', '--num_labels', action='store',
                        dest='num_labels', default='all',
                        help='Number of image labels (categories) to load.')

    parser.add_argument('-d', '--db_name', action='store',
    ...
    global iloader
    iloader = ImageLoader(num_workers=args.num_workers, db_creds)
